...
const ShopInfo = ({ isOwner }) => {
  ...
  const handleSendMessage = async () => {
    if (!user) {
      showToast("error", "Auth Error", "Please login to send a message");
      return;
    }

    if (!data?._id) {
      showToast("error", "Shop Error", "Shop information not available");
      return;
    }

    try {
      // Create conversation with the shop
      const groupTitle = `shop_${data._id}_${user._id}`;
